 ...
 */
public class DRGDetachmentAutoInducerEpsApp extends ModelHandler {
	// output directory name
	private final static String outputDirectory = "D:\\results\\drg-detachment-AI-eps\\";
	...
	// Autoinducer
	private static final float diffusivityAI = 5.76e6f; // [um2/h]

	private static float constantBulkConcentrationAI = 0; // [2.4f g/l]

	private static final float KAI = 3.5e-4f; // [gO2/l]

	private static float productionRateAI = 5e-2f; // [1.4e-3 h-1]
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// auto inducer
		SoluteSpecies autoInducer = new SoluteSpecies("autoInducer",
				diffusivityAI);
		autoInducer.setBulkConcentration(new ConstantBulkConcentration(
				constantBulkConcentrationAI));
		// create the species
		// active mass
		...
		ParticulateSpecies eps = new ParticulateSpecies("eps", densityEPS,
				Color.gray);
		// inert material
		ParticulateSpecies inert = new ParticulateSpecies("inert", densityH,
				Color.gray);
		...
		// growth
		Reaction growth = new Reaction("growth", activeMassH, uMax, 1);
		growth.addFactor(new Saturation(oxygen, KO2));
		// eps decay
		Reaction autoInducerProduction = new Reaction("autoInducerProduction",
				activeMassH, productionRateAI, 1);
		autoInducerProduction.addFactor(new Constant(1.0f));
		// create the maintenance reaction
		Reaction maintenance = new Reaction("maintenance", activeMassH, mO2, 1);
		maintenance.addFactor(new Saturation(oxygen, KO2));
		// inactivation
		Reaction inactivation = new Reaction("inactivation", activeMassH,
				inactivationConstantH, 1);
		inactivation.addFactor(new Inhibition(oxygen, KO2));
		// eps production
		Reaction epsProduction = new Reaction("epsProduction", activeMassH,
				uMax, 2);
		epsProduction.addFactor(new Saturation(oxygen, KO2));
		epsProduction.addFactor(new Saturation(autoInducer, KAI));
		...
		rseps.addReaction(epsProduction, YEPS);
		eps.setProcesses(rseps);
		// inert formation
		NetReaction rinert = new NetReaction(1);
		rinert.addReaction(inactivation, 1.0f);
		inert.setProcesses(rinert);
		// assign reaction stoichiometry to the chemicals
		// oxygen
		NetReaction rsc = new NetReaction(3);
		rsc.addReaction(growth, 1.0f / YH);
		rsc.addReaction(maintenance, 1.0f);
		rsc.addReaction(epsProduction, YEPS / YH);
		oxygen.setProcesses(rsc);
		// auto-inducer
		NetReaction rsai = new NetReaction(1);
		rsai.addReaction(autoInducerProduction, 1.0f);
		autoInducer.setProcesses(rsai);
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new DRGDetachmentAutoInducerEpsApp();
		...
		try {
			...
			String dir = outputDirectory + "umax" + df.format(uMax) + "kdet"
					+ df.format(rdetach) + "rai" + df.format(productionRateAI)
					+ "kai" + df.format(KAI) + "\\";
