 ...
 */
public class CaseStudy2 extends ModelHandler {
	...
	// Enzyme parameters
	private static float epsDecayRate = 1000f; // [gEPS/gPDP/h]
		...
		/*else if (time >= 1) {
			...
			if (time > 40.0f) {
				...
				for (int i = 0; i < _timeSeries.size(); i++) {
					...
					if (time - newTime < 20.0f) {
						indice20 = i - 1;
						break;
					}
				}
				//if the number of particles has increased break
				int nParticlesNew = ((Integer) _particleNumbers.get(indice20))
						.intValue();
