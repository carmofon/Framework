package nl.tudelft.bt.model.tests.twospecies;
 ...
 */
public class DRGConstantDepthDynamic2Species extends ModelHandler {
	...
	private static float inputConcentrationS = 0.001f; //[gCOD/l]
	// N
	private static final float diffusivityN = 7083333.333f; //[um2/h]
	private static final float KN = 0.0015f; //[gN/l]
	private static float inputConcentrationN = 0.006f; //[gN/l]
	//
	// Particulate species
	//Heterotroph
	...
	//Nitrifier
	private static float uMaxN = 0.005775f; // [1/h]
	...
	private static int initialCellNumber = 12; // particles of each specie
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		SoluteSpecies nitrogenSource = new SoluteSpecies("nitrogenSource",
				diffusivityN);
		nitrogenSource.setBulkConcentration(new DynamicBulkConcentrationImplicit(
				inputConcentrationN));
		//
		// create the species
		//heterotrophs
		...
		ParticulateSpecies[] fH = {activeMassH};
		...
		//heterotroph growth
		Reaction growthH = new Reaction("growthH", activeMassH,
				uMaxH, 1);
		growthH
				.addFactor(new MonodTwoSubstrates(oxygen, KO2_H, carbonSource,
						KS));
		//nitrifier growth
		Reaction growthN = new Reaction("growthN", activeMassN,
				uMaxN, 1);
		growthN
				.addFactor(new MonodTwoSubstrates(oxygen, KO2_N, nitrogenSource,
						KN));
		//
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new DRGConstantDepthDynamic2Species();
