 ...
 */
public class ModelWithAngiogenesisSimple extends ModelHandler {
	...
	protected static String outputDirectory = "/Users/CarmonaFontaine/Documents/Xlab/Models/AngiogenesisSimple/Test";
	...
	protected static float relativeMaximumRadius = 15f / systemSize;
	...
	protected static float glucoseBulkConcentration = 5f; //1g/L blood plasma
	...
	// Yield coefficients
	private static float gY = 0.5f; // [gX/gS] Glucose Yield 
	private static float oY = 0.5f; // [gX/gS] Oxygen Yield
	private static float lY = 1f - gY; // [gX/gS] Lactate production Yield
...
//	private static float switchValue = lY * 0.000000000001f;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// growth
		// Tumor growth weight
		ProcessFactor TumorGrowth = new Saturation(glucose, gY);
		
		// Macro growth weight
		ProcessFactor MacroGrowth = new Saturation(glucose, gY);

		// Lactate Effect
		ProcessFactor LactateEffT = new Saturation(lactate, lKST);
		ProcessFactor LactateEffM = new Saturation(lactate, lKSM);
//		ProcessFactor LactateEffE = new Inhibition(lactate, lKSM);

		// 5. Create the reactions
		// Tumor growth
		Reaction GrowthT = new Reaction("GrowthT", activeT, uMaxT, 1);
		GrowthT.addFactor(TumorGrowth);

		Reaction GrowthM = new Reaction("GrowthM", activeM, uMaxM, 1);
		GrowthM.addFactor(MacroGrowth);
		...
		// active mass
		NetReaction rsXactiveT = new NetReaction(2);
		rsXactiveT.addReaction(GrowthT, 1);
		rsXactiveT.addReaction(LactateKillingT, -1);
		activeT.setProcesses(rsXactiveT);

		NetReaction rsXactiveM = new NetReaction(2);
		rsXactiveM.addReaction(GrowthM, 1);
		...
		rsOxygen.addReaction(GrowthT, -(1 / oY));
		rsOxygen.addReaction(GrowthM, -(1 / oY));
		oxygen.setProcesses(rsOxygen);

		NetReaction rsLactate = new NetReaction(3);
		rsLactate.addReaction(GrowthT, (lY));
		rsLactate.addReaction(GrowthM, (lY));
		rsLactate.addReaction(GrowthM,-(1 / clY));
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new ModelWithAngiogenesisSimple();
