 ...
 * @author Will Chang
 */
public class TwoCellFeedbackUnbounded extends GranuleModelHandler {
...
//	protected static float CSF1BulkConcentration = 6e-5f; 
	...
	protected static float maxR = 0.01f; // [dimensionless] (fraction of biomass)
...
//	protected static float relativeMinResponse = 0.1f;
	...
	private static float KD = 0.3f; // [dimless]
	...
	private static float WS = 0.95f; // [dimensionless]
...
//	private static float WS = 0f;
	...
	protected static int tumorCellNumber = 1000;
//	protected static int tumorCellNumber = 0;
	protected static int macrophageNumber = 10;
	
	// initial radius of macrophage inoculum
	protected static float macrophageInoculateRadius = 40f;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// CSF-1 flux
		Reaction fluxCSF1 = new Flux("fluxCSF-1", CSF1, CSF1BulkConcentration);
...
//		inoculateRandomlyAtRadius(nMacrophage, macrophageInoculateRadius);
		inoculateRandomlyInGranuleAtRadius(nMacrophage, macrophageInoculateRadius);
...
//		inoculateRandomlyInsideRadius(nTumor, tumorInoculateRadius);
	...
	public static void writeParameters() throws IOException {
		...
		parameterWriter.writeToFile("Bulk_CSF-1\t" + CSF1BulkConcentration);
		parameterWriter.writeToFile("Bulk_EGF\t" + EGFBulkConcentration);
		parameterWriter.writeToFile("CSF-1_diffusivity\t" + CSF1Diffusivity);
		parameterWriter.writeToFile("EGF_diffusivity\t" + EGFDiffusivity);
		parameterWriter.writeToFile("relative_adhesive_radius\t" + RA);
		parameterWriter.writeToFile("CSF-1_max_production_rate\t" + Rc);
		parameterWriter.writeToFile("EGF_max_production_rate\t" + Re);
		parameterWriter.writeToFile("CSF-1_halfmax_constant\t" + Kc);
		parameterWriter.writeToFile("EGF_halfmax_constant\t" + Ke);
		parameterWriter.writeToFile("min_CSF-1_response\t" + minimumCSF1Response);
		parameterWriter.writeToFile("minimum_EGF_response\t" + minimumEGFResponse);
		parameterWriter.writeToFile("external_mass_transfer_coefficient\t" + externalMassTransferCoefficient);
		parameterWriter.writeToFile("macrophage_WG\t" + macrophageWG);
		parameterWriter.writeToFile("macrophage_WS\t" + macrophageWS);
		parameterWriter.writeToFile("tumor_WG\t" + tumorWG);
		parameterWriter.writeToFile("tumor_WS\t" + tumorWS);
		parameterWriter.writeToFile("macrophage_directedness_constant\t" + macrophageKDirectedness);
		parameterWriter.writeToFile("tumor_directedness_constant\t" + tumorKDirectedness);
		parameterWriter.writeToFile("macrophage_motility\t" + macrophageMotility);
		parameterWriter.writeToFile("tumor_motility\t" + tumorMotility);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length > 0) {
			...
			maxR = Float.parseFloat(args[4]);
			Rc = maxR;
			Re = maxR;
			// limit run length to < system size; unrealistic cluster behavior
			// if cells migrate through cyclic boundaries
			simulationFinishRadius = 0.9f * systemSize;
		}
		MultigridVariable.setSteps(5, 50);
		// create a hande for the application, which will be decorated
		ApplicationComponent app = new TwoCellFeedbackUnbounded();
