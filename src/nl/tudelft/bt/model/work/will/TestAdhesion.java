 ...
 * @author Will Chang
 */
public class TestAdhesion extends GranuleModelHandler {
	...
	protected static float signalBulkConcentration = 1e-7f;
	...
	//private static float signalDiffusivity = 5.76e20f;
	...
	protected static float migratorMotility = 0.02f; // [um^2/h]
	...
	private static float migratorKspread = 0.2f; // [um^-1]
	...
	// weight of social contribution to movement
	private static float ws = 0.9f; // [dimless]
	
	// weight of individual contribution to movement
	private static float wg = 1 - ws; // [dimless]

	// relative adhesive radius of adhesive cells (order of 1)
	private static float relativeAdhesiveRadius = 1.5f; // [dimless]
	...
	protected static float relativeMaximumRadius = 0.01f;
	...
	protected static float relativeBoundaryLayer = 0.04f;
	...
	protected static int initialAttractorNumber = 10;
	//protected static int initialAttractorNumber = 0;
	
	// initial radius of inoculum
	protected static float attractorRadius = 20f;
...
//				oxygenBulkConcentration));
		...
		AdhesiveBiomassSpecies speciesMigrator = new AdhesiveBiomassSpecies("migrator",
				spMigrator, fractionalVolumeCompositionMigrator, signal,
				migratorMotility, migratorKspeed, migratorKspread, 
				ws, wg, relativeAdhesiveRadius);
		...
		//growthMigrator.addFactor(signalS);
		...
		rsSignal.addReaction(growthMigrator, -(1 / YXS));
	...
	protected void inoculate() {
		int[] nMigrator = { initialMigratorNumber, 0 };
		int[] nAttractor = { 0, initialAttractorNumber };
		inoculateRandomlyAtRadius(nAttractor, attractorRadius);
		inoculateRandomly(nMigrator);
	}
	...
	public static void main(String[] args) {
		...
		if (args.length < 6) {
			throw new RuntimeException(
					"input arguments missing: \n"
							...
							+ "5: number of migrator cells in inoculum"
							+ "6: weight of social contribution to movement"
			);
		}
		
		outputDirectory = args[0];
		boolean runWithGraphics = (Integer.parseInt(args[1]) == 1);
		geometry = Integer.parseInt(args[2]);
		int seed = Integer.parseInt(args[3]);
		Model.model().setSeed(seed);
		initialMigratorNumber = Integer.parseInt(args[4]);
		ws = Float.parseFloat(args[5]);
		wg = 1 - ws;
		...
		ApplicationComponent app = new TestAdhesion();
