...
public class sandwichversionLac extends GranuleModelHandler {
	...
	protected static String outputDirectory = "C:/Javaresults/Parameter testing 2/test2/";
	...
	protected static float oxygenBulkConcentration = 5e-3f; // [g/L] This value might be an overestimation (distilled H2O)
	protected static float glucoseBulkConcentration = 3f; // [g/L]
	protected static float lactateBulkConcentration = 0f; // [g/L]

// in the paper Ds = 1.6e-9 m2/s = 5.76e6 um2/h
//	private static float OxygenDiffusivity = 1e6f;
//	private static float substrateDiffusivity = 1e6f;
	private static float oxygenDiffusivity = 7e6f; // [um^2/h] 0.0000197[cm2/s] http://compost.css.cornell.edu/oxygen/oxygen.diff.water.html
	private static float substrateDiffusivity = 6e6f; // [um^2/h] I found 600[um^2/s], which would lead to 2.2e6 I do not think this is a significant difference but we have to consider this is probably an overestimation
	// 
	...
	// Yield coefficients
	private static float YOxy = 0.04f; // [gX/gS] 71%
	private static float YGluaer = 0.24f; // [gX/gS] 24%
	private static float YGluana = 0.06f; // [gX/gS] 6% (24/6fold)
	private static float YLac = 0.03f; // [gX/gS] All the mass of Glu consumed in fermentation (1/0.06)
	
	// Processes
	// Growth (biomass production)
	//Beetween 20 and 15 hrs generation time   Tzur et. al. says Science. 2009 July 10; 325(5937): 167–171. doi:10.1126/science.1174294.
	protected static float uMaxX = 0.04f; // [1/h]
	protected static float uMaxY = 0.04f; // [1/h]

	// protected static float uMax = 0.0547f; //[1/h]

	private static float KSOxy = 1e-4f; // [g/L]       I've put all the same and very low for now
	private static float KSGluaer = 1e-4f; // [g/L]
	private static float KSGluana = 1e-3f; // [g/L]
	private static float KSLac = 2f; // [g/L]
	//private static float KSY = 3.5e-4f; // [g/L]

	private static float kDeathX = 0.03f; // [h-1]
	private static float kDeathY = 0.04f; // [h-1]
	//private static float kDeathY = 0.001f; // [h-1]

	// Computation parameters
	protected static float systemSize = 3400; // [um]
	...
	protected static float relativeMinimumRadius = relativeMaximumRadius * 0.3f;
		

	// Defines the thickness of the concentration boundary layer in the system.
	// Here, the thickness of the boundary layer is 0.1*2000 = 200 um
	protected static float relativeBoundaryLayer = 0.03f;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		ProcessFactor OaerS = new Saturation(Oxygen, KSOxy);
		ProcessFactor OanaS = new Inhibition(Oxygen, KSOxy);
		ProcessFactor GaerS = new Saturation(Glucose, KSGluaer);
		ProcessFactor GanaS = new Saturation(Glucose, KSGluana);
		ProcessFactor LacS = new Saturation(Lactate, KSLac);
		...
		Reaction deathX = new Reaction("deathX", activeX, kDeathX, 0);
		...
		Reaction deathY = new Reaction("deathY", activeY, kDeathY, 0);
		...
		rsLactate.addReaction(growthanaX, (1  /YLac));
		rsLactate.addReaction(growthanaY, (1 / YLac));
	...
	public static void main(String[] args) {
		...
		ApplicationComponent app = new sandwichversionLac();
