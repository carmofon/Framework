 ...
 */
public class Example7ESSAllInitialDifferentToxinDifferentLevel extends ModelHandler {
	...
	static int numberOfToxinLevels = 10;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// X active mass
		ParticulateSpecies[] activeX = new ParticulateSpecies[numberOfToxinLevels+1];
		...
		activeX[numberOfToxinLevels] = inertX;
		
		float[] fractionalVolumeCompositionH1 = new float[numberOfToxinLevels+1];
		float initialComposition = 1.0f/(1.0f+(float)(numberOfToxinLevels));
		for (int i = 0; i < numberOfToxinLevels; i++){
			fractionalVolumeCompositionH1[i] = initialComposition ;	
		}
		//fractionalVolumeCompositionH1[0] = 1.0f/numberOfToxinLevels;
		
		// 3. Create the Reaction factors, Monod and inhibition coefficients
		ProcessFactor mO = new Saturation(oxygen, KO);
		
		//4. Create the mutators biomass specie
		speciesX = new BiomassSpecies("mutator",
				activeX, fractionalVolumeCompositionH1);
		speciesX.setEpsMass(inertX);
		...
		// inert
		NetReaction rsXinert = new NetReaction(numberOfToxinLevels);
		for (int i = 0; i < numberOfToxinLevels; i++) {
			rsXinert.addReaction(toxinKilling[i], 1);
		}
	...
	public static void main(String[] args) throws IOException {
		...
		if (args.length < 11) {
			throw new RuntimeException(
					"input arguments missing: \n"
							...
							+ "8: Oxygen Diffusivity \n"
							+ "9: Number of Toxin Levels"
							+ "10: Mutation Rate \n"
							+ "11: Steady State height h*"	);
		}
		// parse inputs
		...
		oxygenBulkConcentration = Float.parseFloat(args[6]);
		oxygenDiffusivity = Float.parseFloat(args[7]);
		numberOfToxinLevels =  Integer.parseInt(args[8]);
		mutationRate = Float.parseFloat(args[9]);
		h_star = Float.parseFloat(args[10]);
		...
		* (relativeMaximumRadius * systemSize) * systemSize / gridSide * specificMassX;
		...
		ApplicationComponent app = new Example7ESSAllInitialDifferentToxinDifferentLevel();
