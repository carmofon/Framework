 ...
 */
public class PublicGoodCooperation3 extends ModelHandler {
	...
	protected static String outputDirectory = "/Users/bucciv/results/PC1/";
	...
	// Solute species
	// Nutrient (N) - the only solute species used here
	protected static float nutrientBulkConcentration = 8e-16f; // [g/L]
	// in the paper Ds = 1.6e-9 m2/s = 5.76e6 um2/h
	private static float nutrientDiffusivity = 8.33e-8f; // [um^2/h]

	// (PG)-PUBLIC GOOD bulk concentration (0 initial)-V.B.
	protected static float PGBulkConcentration = 0.0f; // [g/L]
	// PG - PUBLIC GOOD diffusivity
	private static float PGDiffusivity = 0.03f ; //8.33e6f; // [um^2/h]
	private static float PGthreshold;
	...
	// Yield coefficients
	private static float Y = 0.5f; // [gX/gN]
	private static float YO = 0.5f; // [gO/gX]
	//-1/Y paper = Y/(Y0(1-Y))
	
	// Processes
	// Growth (biomass production)
	protected static float qsMax = 2.0f;//2.00f; // [1/h]   MUMAX = qsmax*Y = 0.88
	private static float Kn = 1.18e-3f; // half saturation constant [g/L]
	// protected static float uMax = 0.0547f; //[1/h]

	// PG production
	protected static float f;
	protected static float PGInvestment; 
	// PG efficiency
	protected static float kpg = 5;// [L/gS/hr]
	// Toxin loss rate per unit of susceptible species concentration
	protected static float alpha = 1.0f; // (gPG/gX) PG to Biomass conversion
	
	private static float kd=0.01f;
											// factor
	// Length Scale Ab
	private static float Ab; // Ab=kpg*rx*alpha/(4*pi*Ds)
	
	// Computation parameters
	protected static float systemSize = 250/2f; // [um]
	...
	protected static float relativeBoundaryLayer = 1f / systemSize;
	...
	private static BiomassSpecies Producers;
	private static BiomassSpecies Cheaters;
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// Oxygen species
		SoluteSpecies nutrient = new SoluteSpecies("Nutrient", nutrientDiffusivity);
		// set up the simplest type of bulk concentration: constant
		nutrient.setBulkConcentration(new ConstantBulkConcentration(
				nutrientBulkConcentration));
		// Toxin species
		SoluteSpecies PG = new SoluteSpecies("Public_Good", PGDiffusivity);
		// set up the simplest type of bulk concentration: constant
		PG.setBulkConcentration(new ConstantBulkConcentration(
				PGBulkConcentration));
		...
		activeX[0]=new ParticulateSpecies("activeX",
		...
		activeX2[0]=new ParticulateSpecies("activeX2",
		...
		ParticulateSpecies[] spX = { activeX[0], activeX[1] };
		float[] fractionalVolumeCompositionH1 = { 1.0f, 0f};
		
		// array of fixed species that constitute speciesX (in this case,
		// speciesX is entirely constituted by active mass)
		ParticulateSpecies[] spX2 = { activeX2[0], activeX2[1] };
		float[] fractionalVolumeCompositionH2 = { 1.0f, 0f };
		...
		PGInvestment = f*qsMax*Y;
		
		ProcessFactor mO = new Saturation(nutrient, Kn);
		
		Cheaters = new BiomassSpecies("Cheaters", spX,
				fractionalVolumeCompositionH1);
		Cheaters.setActiveMass(activeX[0]);
		Cheaters.setEpsMass(activeX[1]);

		// 3.1 V.B. Create the second biomass specie
		Producers = new BiomassSpecies("Producers", spX2,
				fractionalVolumeCompositionH2);
		Producers.setActiveMass(activeX2[0]);
		Producers.setEpsMass(activeX2[1]);
		...
		// growth for Chaters
		Reaction growthCheaters = new Reaction("growth", activeX[0], qsMax, 1);
		growthCheaters.addFactor(mO);
		// This creates a growth rate that equals:
		// rX = uMax*Cs/(Cs+KS)*Cx
		// where Cx is the concentration of biomass
		//
		// similarly for Producers
		Reaction growthProducers = new Reaction("growthX2", activeX2[0], qsMax, 1);
		growthProducers.addFactor(mO);

		// PG production from producers
		Reaction PGProduction = new Reaction("PGProduction", activeX2[0],
				PGInvestment, 1);

		// PG growth stimulation
		Reaction PGsensingCheater = new Reaction("PGsensingCheater", activeX[0], kpg, 1);
		ProcessFactor T1 = new Linear1(PG);
		PGsensingCheater.addFactor(T1);
		
		Reaction PGsensingProducer = new Reaction("PGsensingProducer", activeX2[0], kpg, 1);
		ProcessFactor T2 = new Linear1(PG);
		PGsensingProducer.addFactor(T2);
		
		
		// Endogenous Decay

		Reaction BiomassDecayProducer = new Reaction ("BiomassDecayProducer", activeX2[0], kd, 0);
		Reaction BiomassDecayCheater = new Reaction ("BiomassDecayCheater", activeX[0], kd, 0);
		
		// 6. Assign reaction to the species through ReactionStoichiometries
		// active mass
		NetReaction rsXactiveX = new NetReaction(3);
		rsXactiveX.addReaction(growthCheaters, Y);
		rsXactiveX.addReaction(PGsensingCheater, 1);
		rsXactiveX.addReaction(BiomassDecayCheater,-1);
		activeX[0].setProcesses(rsXactiveX);

		// similarly for activeX2 growth and investement in Toxin production
		NetReaction rsXactiveX2 = new NetReaction(4);
		rsXactiveX2.addReaction(growthProducers, Y);
		rsXactiveX2.addReaction(PGProduction, -1);
		rsXactiveX2.addReaction(PGsensingProducer, 1);
		rsXactiveX2.addReaction(BiomassDecayProducer, -1);
		activeX2[0].setProcesses(rsXactiveX2);
		
		
		// inert
		NetReaction rsXinert = new NetReaction(1); 
		rsXinert.addReaction(BiomassDecayCheater, 1);
		inertX.setProcesses(rsXinert);
		
		NetReaction rsXinert2 = new NetReaction(1); 
		rsXinert2.addReaction(BiomassDecayProducer, 1);
		...
		// oxygen
		NetReaction rsNutrient = new NetReaction(2);
		rsNutrient.addReaction(growthCheaters, -(YO * (1 - Y)));
		rsNutrient.addReaction(growthProducers, -(YO * (1 - Y)));
		nutrient.setProcesses(rsNutrient);

		// Toxin
		NetReaction rsPG = new NetReaction(1);
		rsPG.addReaction(PGProduction, alpha);
		PG.setProcesses(rsPG);
		...
		addBiomassSpecies(Cheaters);
		addBiomassSpecies(Producers);
	...
	public static void main(String[] args) throws IOException {
		...
		kpg = Float.parseFloat(args[3]);
		PGDiffusivity=Float.parseFloat(args[4]);
		geometry = Integer.parseInt(args[5]);
		f = Float.parseFloat(args[6]);
		alpha=Float.parseFloat(args[7]);
		specificMassX=Float.parseFloat(args[8]);
		InitialRatio =  Float.parseFloat(args[9]);
		nutrientBulkConcentration= Float.parseFloat(args[10]);
		nutrientDiffusivity= Float.parseFloat(args[11]);
		kd=Float.parseFloat(args[12]);
		...
		* (relativeMaximumRadius * systemSize) * systemSize / gridSide * specificMassX;
        ...
        // compute the length scale Ab from the parameters
		if (Model.model().getDimensionality() == 3) {
		Ab = (alpha * kpg * (cellmassmax + cellmassmin) / 2.0f)
		/ (4.0f * 3.1415f * PGDiffusivity);
		}else{
			Ab = (alpha * kpg * (cellmassmax + cellmassmin) / 2.0f)
			/ (2.0f * 3.1415f * PGDiffusivity);
		}
		
		Float delta_square = new Float (nutrientBulkConcentration * nutrientDiffusivity * Y/(YO*(1-Y))) / ((qsMax * Y) * specificMassX * (relativeBoundaryLayer * systemSize) * (relativeBoundaryLayer * systemSize));
		Delta = Math.sqrt(delta_square.doubleValue());
		//
		MultigridVariable.setSteps(5, 50);
		// create a hande for the application, which will be decorated
		ApplicationComponent app = new PublicGoodCooperation3();
		...
		try {
			...
			Parameters.writeToFile("Ab " + Ab);
			Parameters.writeToFile("Alpha: " + alpha);
			Parameters.writeToFile("kpg: " + kpg);
			Parameters.writeToFile("Diffusivity: " + PGDiffusivity);
			Parameters.writeToFile("f: " + f);
			...
			spw.addSeries(new ComputeRelatednessTermSeries(Cheaters, Producers));
			//spw.addSeries(new SegregationIndex(Producers, Cheaters, Ab));
			...
			// add the number of total individuals
			spw.addSeries(new NumberofCellsTermSeries(Cheaters, Producers));
			//Model.model().setMaximumTimeStep(1);
			//Model.model().setFinishIterationTime(1);
			Model.model().setMaxHeight(90);
