 ...
 */
public class Example4ToxinOxygen extends ModelHandler {
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		ToxinMinus = new BacteriocinBiomassSpecies("ToxinMinus", spX,
				fractionalVolumeCompositionH1, Toxin, Toxin_Investment, mO,
		...
		Reaction growth;
		if (nutrientOff)
			growth = new Reaction("growth", activeX, qsMax, 0);
		else {
			growth = new Reaction("growth", activeX, qsMax, 1);
			growth.addFactor(mO);
		}
		...
		Reaction growthX2;
		if (nutrientOff)
			growthX2 = new Reaction("growthX2", activeX2, qsMax, 0);
		else {
			growthX2 = new Reaction("growthX2", activeX2, qsMax, 1);
			growthX2.addFactor(mO);
		}	
		// Toxin production
		// Reaction ToxinProduction = new Reaction("ToxinProduction", activeX2,
		// qT, 1);
		Reaction ToxinProduction;
		if (nutrientOff)
			ToxinProduction = new Reaction("ToxinProduction", activeX2,
					Toxin_Investment, 1);
		else {
				ToxinProduction = new Reaction("ToxinProduction", activeX2,
				Toxin_Investment, 1);
		...
		// rsOxygen.addReaction(growth, -1/Y);
		// rsOxygen.addReaction(growthX2, -1/Y);
	...
	public static void main(String[] args) throws IOException {
		...
		// Toxinthreshold = qT/kT; With oxygen gradients the ToxinThreshold is
		// not fixed but varies: T* = fqsmaxO2/(kt(O2+KO2))

		// Float delta_square = new Float (oxygenBulkConcentration *
		// oxygenDiffusivity * Y/(YO*(1-Y))) / ((qsMax * Y) * specificMassX *
		// (relativeBoundaryLayer ) * (relativeBoundaryLayer ));
		Float delta_square = new Float(oxygenBulkConcentration
		...
		ApplicationComponent app = new Example4ToxinOxygen();
		...
		System.out.println("Toxin Investment is:" + Toxin_Investment);
		try {
			// create the space
