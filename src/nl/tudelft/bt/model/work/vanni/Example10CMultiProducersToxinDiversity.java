 ...
 */
public class Example10CMultiProducersToxinDiversity extends ModelHandler {
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// Producers
		// for each victim create an array with all the Toxin concentrations from the other producers
		// to pass to the LinearWithSpeciesArray reaction
		Reaction [] toxinKillingProducers = new Reaction[numberofCellGroups - 1];
		SoluteSpecies[] ToxinArrayKillerProducer= new SoluteSpecies [numberofCellGroups-1];
		for (int victim = 0; victim < numberofCellGroups - 1; victim++) {
			toxinKillingProducers[victim] = new Reaction(
					"ToxinKillingOf" + victim,
					activeX[victim], kT, 1);
			for (int killer = 0; killer < numberofCellGroups - 1; killer++) {
				if (killer != victim) {
					ToxinArrayKillerProducer[killer]=Toxin[killer];
				} 
			}
			toxinKillingProducers[victim].addFactor(new LinearWithSpeciesArray(ToxinArrayKillerProducer));
		...
		SoluteSpecies[] ToxinArrayKillerSensitive= new SoluteSpecies [numberofCellGroups-1];
		Reaction toxinKillingSensitive = new Reaction(
				"ToxinKillingOfSensitive",
				activeX[numberofCellGroups - 1], kT, 1);
		for (int killer = 0; killer < numberofCellGroups - 1; killer++) {
			ToxinArrayKillerSensitive[killer]=Toxin[killer];
		}
		toxinKillingSensitive.addFactor(new LinearWithSpeciesArray(ToxinArrayKillerSensitive));
		...
		for (int victim = 0; victim < numberofCellGroups - 1; victim++) {
			rsXactive[victim] = new NetReaction(4);
			rsXactive[victim].addReaction(growth[victim], Y);
			rsXactive[victim].addReaction(toxinKillingProducers[victim], -1);
		...
		// sensitive
		rsXactive[numberofCellGroups - 1] = new NetReaction(3);
		rsXactive[numberofCellGroups - 1].addReaction(
				growth[numberofCellGroups - 1], Y);
		rsXactive[numberofCellGroups - 1].addReaction(
				toxinKillingSensitive, -1);
		rsXactive[numberofCellGroups - 1].addReaction(
		...
		for (int victim = 0; victim < numberofCellGroups - 1; victim++) {
			...
			rsXinert[victim] = new NetReaction(2);
			rsXinert[victim].addReaction(toxinKillingProducers[victim], 1);
			rsXinert[victim].addReaction(BiomassDecay[victim], 1);
			inertX[victim].setProcesses(rsXinert[victim]);
		}
		// sensitive
		rsXinert[numberofCellGroups - 1] = new NetReaction(2);
	...
	public static void main(String[] args) throws IOException {
		...
		ApplicationComponent app = new Example10CMultiProducersToxinDiversity();
