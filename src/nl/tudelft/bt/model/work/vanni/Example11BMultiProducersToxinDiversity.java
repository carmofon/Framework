 ...
 */
public class Example11BMultiProducersToxinDiversity extends ModelHandler {
	...
	private void defineSpeciesAndReactions() throws ModelException {
		...
		// for producers
		Reaction[][] toxinKillingProducers = new Reaction[numberofCellGroups][numberofCellGroups];
			...
			for (int victim = 0; victim < numberofCellGroups; victim++) {
				if (killer != victim) {
					toxinKillingProducers[killer][victim] = new Reaction(
							...
							activeX[victim], kT, 1);
					toxinKillingProducers[killer][victim].addFactor(new Linear1(Toxin[killer]));
				}
			}
		}
		...
		for (int victim = 0; victim < numberofCellGroups; victim++) {
			rsXactive[victim] = new NetReaction(3 + (numberofCellGroups - 1));
		...
		for (int victim = 0; victim < numberofCellGroups; victim++) {
			rsXinert[victim] = new NetReaction(1 + numberofCellGroups - 1);
			for (int killer = 0; killer < numberofCellGroups; killer++) {
				if (killer != victim) {
	...
	public static void main(String[] args) throws IOException {
		...
		ApplicationComponent app = new Example11BMultiProducersToxinDiversity();
