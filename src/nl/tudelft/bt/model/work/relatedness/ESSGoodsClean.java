 ...
 */
public class ESSGoodsClean extends ModelHandler {
	...
	public static void main(String[] args) {
		...
		goodRateCost1 = Float.parseFloat(args[3]);
		goodRateCost2 = 0;


		int nutrientCondition = Integer.parseInt(args[5]);
		int dimensionallity = Integer.parseInt(args[6]);
		float durationMultiplier = 1;
		int inoculationMultiplier = 1;
		...
		if (Integer.parseInt(args[4]) == 1) {
			initialParticleNumber1 = 60*inoculationMultiplier;
			initialParticleNumber2 = 60*inoculationMultiplier;
		} else {
		...
		if (nutrientCondition == 1) {
			...
			simulationFinishTime = 45f*durationMultiplier; // [h]
			...
			if (nutrientCondition == 2) {
				...
				simulationFinishTime = 22f*durationMultiplier; // [h]
				outputEvery = simulationFinishTime - .01f;
				gradientsOn = true;
			} else {
				// No nutrient gradients
				substrateBulkConcentration = 2e-1f; // [gO2/L] DO 20
				simulationFinishTime = 4.4f*durationMultiplier; // [h]
				outputEvery = simulationFinishTime - .01f;
				gradientsOn = false;
			}
		}
		...
		ApplicationComponent app = new ESSGoodsClean();
